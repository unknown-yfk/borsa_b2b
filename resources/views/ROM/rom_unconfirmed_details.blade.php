@extends('layouts.mainlayout')
@section('content')
    <div class="main-panel">
        <div class="content-wrapper">




            <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Order History</h4>

                        <p class="card-description">
                            <code>List of orders</code>
                        </p>
                        <input type="hidden" value="{{ $new=0 }} ">
                        <div class="table-responsive pt-3">
                            {{-- <table id="datatable" class="table"> --}}
                            <form method="POST" action="{{ route('ordered-products.confirm') }}">
                                @csrf

                                <table id="recent-purchases-listing" class="table">

                                    <thead>
                                        <tr>
                                            <!-- Set columns width -->
                                            <th class="text-center py-3 px-4">Product Name &amp; Details</th>
                                            <th class="text-right py-3 px-4">Current Unit Price</th>
                                            <th class="text-right py-3 px-4">Unit Price</th>
                                            <th class="text-center py-3 px-4">Ordered Quantity</th>
                                            <th class="text-right py-3 px-4">Sub-Total </th>
                                            <th class="text-right py-3 px-4">Action </th>

                                        </tr>
                                    </thead>
                                    <tbody>

                                        @foreach ($orderedProducts as $item)
                                            <tr>
                                                <td class="p-4">
                                                    <div class="media align-items-center">
                                                        <img src="{{ asset('/assets/product_img/' . $item->image) }}"
                                                            class="d-block ui-w-40 ui-bordered mr-4" alt="">
                                                        <div class="media-body uppercase">
                                                            {{ $item->name }}<br>
                                                            <span class="text-muted"><strong> Description:</strong> </span>
                                                            {{ $item->description }}
                                                        </div>
                                                    </div>
                                                </td>
                                                <td class="text-right font-weight-semibold align-middle p-4">
                                                    {{ $current = $item->price }}
                                                    br</td>
                                                <td class="text-right font-weight-semibold align-middle p-4">
                                                    {{ $ordered_price = $item->subTotal / $item->ordered_quantity }}
                                                    br</td>

                                                <td class="align-middle p-4"><input type="number"
                                                        class="form-control text-center"
                                                        value="{{ $item->ordered_quantity }}" readonly></td>
                                                <td class="text-right font-weight-semibold align-middle p-4">
                                                    {{ $item->subTotal }} br</td>

                                                <td class="text-right font-weight-semibold align-middle p-4">

                                                    {{-- @method('PUT') --}}



                                                    <input type="hidden" value="{{ $item->order_id }}" name="order_id">
                                                    <input type="hidden" value="{{ $item->id }}"
                                                        name="ordered_product_id">
                                                    <input type="hidden" value="{{ $item->ordered_quantity }}"
                                                        name="ordered_quantity">
                                                    <input type="hidden"
                                                        value="{{ $new=$new +$item->price * $item->ordered_quantity }}"
                                                        >



                                                    <input type="hidden" value="{{ $item->id }}"
                                                        name="ordered_product_id">

                                                    {{-- {{ $item->id }} --}}
                                                    <div class="row">
                                                        <div class="col-md-6">
                                                            <div class="form-group">
                                                                <div class="form-check">
                                                                    <label class="form-check-label">
                                                                        <input type="radio"
                                                                            onclick="javascript:yesnoCheck('{{ $item->id }}qa','{{ $item->id }}ifYes');"
                                                                            class="form-check-input"
                                                                            name="status_{{ $item->id }}"
                                                                            id="{{ $item->id }}ac"
                                                                            value="Acceptance"{{ $item->status == 'acceptance' ? 'checked' : '' }}
                                                                            required>
                                                                        Acceptance
                                                                    </label>
                                                                </div>
                                                                <div class="form-check">
                                                                    <label class="form-check-label">
                                                                        <input type="radio"
                                                                            onclick="javascript:yesnoCheck('{{ $item->id }}qa','{{ $item->id }}ifYes');"
                                                                            class="form-check-input"
                                                                            name="status_{{ $item->id }}"
                                                                            id="{{ $item->id }}re"
                                                                            value="Refusal"{{ $item->status == 'refusal' ? 'checked' : '' }}>
                                                                        Refusal
                                                                    </label>
                                                                </div>

                                                                <div class="form-check">
                                                                    <label class="form-check-label">
                                                                        <input type="radio"
                                                                            onclick="javascript:yesnoCheck('{{ $item->id }}qa','{{ $item->id }}ifYes');"
                                                                            class="form-check-input"
                                                                            name="status_{{ $item->id }}"
                                                                            id="{{ $item->id }}qa"
                                                                            value="quantity_Adjustment"{{ $item->status == 'quantity_Adjustment' ? 'checked' : '' }}>
                                                                        Quantity Adjustment

                                                                    </label>


                                                                    <div id="{{ $item->id }}ifYes"
                                                                        style="visibility:hidden">
                                                                        <code>Enter the quantity you have:
                                                                        </code><br><br><input type='number' id='yes'
                                                                            name="quantity_{{ $item->id }}"
                                                                            min="0"
                                                                            max="{{ $item->ordered_quantity }}"><br>
                                                                    </div>


                                                                </div>
                                                            </div>
                                                        </div>






                                                </td>



                                            </tr>
                                        @endforeach

                                    </tbody>
                                </table>
                                {{-- </div> --}}
                                <div class="d-flex flex-wrap justify-content-between align-items-center pb-4">
                                    <div class="mt-4"> </div>
                                    <div class="d-flex">
                                        <div class="text-right mt-4">


                                            <div class="text-large"><strong>Total Price: {{ $item->totalPrice }}
                                                    br</strong></div>

                                        </div>
                                    </div>
                                </div>
                                 <input type="hidden" value="{{ $new }}" name="subtotal">
                                  <input type="hidden" value="{{ $item->totalPrice }}" name="total">

                                <button type="submit" class="btn btn-outline-primary mt-4" id="">Confirm
                                    Order</button><br>
                                {{-- <div class="float-right"> --}}
                                {{-- @if ($item->confirmStatus == 'unconfirmed' && Auth::user()->userType == 'key distributor') --}}
                                {{-- <form action="/confirmOrder/update/edit" method="POST" id="confirmOrder">
                                @csrf
                                @method('PUT')
                                <input type="hidden" value="{{ $item->order_id }}" name="order_id">
                                <input type="hidden" value="confirmed" name="confirm">
                            </form> --}}



                                {{-- </div> --}}

                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </div>


        <script>
            $(document).ready(function() {
                $('input[type=radio]').change(function() {
                    var statusValue = $(this).val();


                    var orderedProductId = $(this).attr('name').split('_')[1];

                    $.ajax({
                        url: '/ordered-products/' + orderedProductId + '/update-status',
                        type: 'POST',
                        data: {
                            status: statusValue,
                            _token: '{{ csrf_token() }}'
                        },
                        success: function(response) {
                            console.log(response);
                        },
                        error: function(error) {
                            console.log(error);
                        }
                    });
                });
            });





            // function text(x) {
            //     if (x == 2) document.getElementById($item - > id).style.display = "block";

            //     else document.getElementById($item - > id).style.display = "none";
            //     return;

            // }
            function yesnoCheck(q, y) {
                console.log(document.getElementById(q).checked);
                console.log(document.getElementById(y).checked);

                console.log(q);
                if (document.getElementById(q).checked) {
                    console.log(y);
                    document.getElementById(y).style.visibility = 'visible';
                } else document.getElementById(y).style.visibility = 'hidden';

            }

            function price_check(current, previous) {
                console.log(current);

                if (current > previous) {

                    document.getElementById(change).style.visibility = 'visible';
                } else document.getElementById(change).style.visibility = 'hidden';

            }
        </script>
    @endsection
