                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex flex-wrap justify-content-between align-items-center  pb-4">
                            <div class="mt-4 btn-group">
                                ...
                                <form action="/rom/return/accept" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <input type="hidden" value="{{ $item->order_id }}" name="order_id">
                                    <input type="hidden" value="confirmed" name="confirm">
                                    <button type="submit" class="btn btn-outline-success mx-2">Accept</button>
                                </form>
                                <br>
                                <form action="/rom/return/decline" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <input type="hidden" value="{{ $item->order_id }}" name="order_id">
                                    {{-- <input type="hidden" value="confirmed" name="confirm"> --}}
                                    <button type="submit" class="btn btn-outline-danger mx-2">Decline</button>
                                </form>
                                {{-- @endif --}}
