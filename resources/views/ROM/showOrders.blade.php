                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="table-responsive pt-3">
                            ...
                            <form method="post" action="{{ route('rom_edit') }}">
                                ...
                                <table id="table" class="display nowrap" style="width:100%">
                                    <thead>
                                        ...
                                        <tr>
                                        <th style="width: 35px; white-space: normal; word-wrap: break-word;">Order Id</th>
<th style="width: 35px; white-space: normal; word-wrap: break-word;">Order Date</th>
<th style="width: 35px; white-space: normal; word-wrap: break-word; ">Agent Name</th>
...
@if (!empty($product) && $product->count())
    @foreach ($product as $order)
        <th style="width: 45px; white-space: normal; word-wrap: break-word;">{{ $order->name }}</th>
    @endforeach
    <th style="width: 45px; white-space: normal; word-wrap: break-word;">Subtotal</th>
@else
                                    ...
                                    <tbody>
                                        ...
                                        @if (!empty($client) && $client->count())
                                            ...
                                            @for ($i = 0; $i < sizeof($client); $i++)
                                                <tr>
                                                    <td style="width: 35px; white-space: normal; word-wrap: break-word;" >{{ $client[$i]->id }}</td>
                                                    <td style="width: 35px; white-space: normal; word-wrap: break-word; ">{{ $client[$i]->created_at }}</td>
                                                    <td style="width: 35px;  white-space: normal; word-wrap: break-word;">{{ $client[$i]->firstName }} {{ $client[$i]->middleName }}
                                                        {{ $client[$i]->lastName }}</td>
                                                    <td style="width: 35px; white-space: normal; word-wrap: break-word;">{{ $client[$i]->Region }}</td>
                                                    <td style="width: 35px; white-space: normal; word-wrap: break-word;">{{ $client[$i]->City }}</td>
                                                    @php
                                                        $amount = 0;
                                                        $price = 0;
                                                ...
                                                $singleProductPrice=$price/$amount;
                                                if($singleProductPrice != $products_pr[$r]){
                                                    $checker=1;
                                                    DB::table('orders')->where('id', $client[$i]->id)->update(['price_update' => 1]);
                                                    }
                                            ...
                                            @endphp
                                                            ...
                                                            <div class="checkbox-container">
                                                                ...
                                                                <input type="number"
                                                                    name="products[{{ $i }}][{{ $j }}][quantity]"
                                                                    class="amount_input quantityInput" min="0"
                                                                    max="{{ $amount }}" />
                                                ...
                                                @endfor
                                                <td style="width: 55px;">{{ $client[$i]->totalPrice }}</td>
                                            </tr>
                                            <input type="hidden" {{ $total = $total + $client[$i]->totalPrice }} />
                                        @endfor
                                    @endif
