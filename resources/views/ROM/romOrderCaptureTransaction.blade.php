...
<div class="main-panel">
    <div class="content-wrapper">
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">BORSA ORDER CAPTURE TRANSACTION SUMMARY REPORT </h4>
                    ...
                    <div class="table-responsive pt-3">
                        <form action="{{ route('romordercapturetransaction') }}" method="GET" class="filter-form" id="filterForm">
...
@endphp
                                   @for ($j = 0; $j < sizeof($product); $j++)
                                    @php
                                    $products_id=explode(',', $lastMileReport[$i]->product_ids);
                                    $subtotals=explode(',', $lastMileReport[$i]->subTotal);
                                    @endphp
                                        @for ($r = 0; $r < sizeof($products_id); $r++)
                                            @if ($product[$j]->id == $products_id[$r])
                                            @php
                                            $quantities = explode(',', $lastMileReport[$i]->ordered_quantities);
                                            $amount=$quantities[$r];
