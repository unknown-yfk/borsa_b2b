        ...
        <div class="main-panel">
            <div class="content-wrapper">
                ...
                <div class="col-lg-12 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title">Handover Hierarchy</h4>

                            <p class="card-description">
                                <code>Check the hierarchy for the order included</code>
                            </p>

                            <table id="hierarchy-listing" class="table">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    @if (!empty($hierarchy) && $hierarchy->count())
                                        @foreach ($hierarchy as $hierarchy)
                                            <tr>
                                                <td><strong>{{ $hierarchy->name }}</strong></td>
                                                <td>
                                                    <input data-id="{{ $hierarchy->id }}" class="toggle-class"
                                                        type="checkbox" data-onstyle="success" data-offstyle="danger"
                                                        data-toggle="toggle" data-on="active" data-off="inactive"
                                                        {{ $hierarchy->status ? 'checked' : '' }}>
                                                </td>
                                            </tr>
                                        @endforeach
                                    @else
...
@endif --}}
        ...
        <script>
            $(document).ready(function() {
                ...
                //             var html = '<input data-id="' + full.id +
                ...
                $('.toggle-class').change(function() {
                    ...
                    var hierarchy_id = $(this).data('id');


                    $.ajax({
                        type: 'GET',
                        dataType: 'json',
                        url: '/admin/hierarchy/changeStatus',
                        data: {
                            'status': status,
                        ...
                        success: function(data) {
                            ...
                            var table = $('#hierarchy-listing').DataTable();
                            var rowData = table.row($(this).closest('tr')).data();
                            rowData[1] = status ? 'active' : 'inactive';
