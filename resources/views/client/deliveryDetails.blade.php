...
@section('content')
 ...
 <div class="col-lg-12 grid-margin stretch-card">
            ...
            @if($item->confirmation_status=='unconfirmed')
              <form action="/client/confirmDelivery/update/edit" method="POST">
              @csrf
              @method('PUT')
                <input type="hidden" value="{{$item->delivery4_id}}" name="delivery_4_id">
              ...
              <button type="submit" class="btn btn-outline-success mt-2">Confirm Order</button>
                ...
                <!-- @elseif($item->confirmationStatus=='confirmed'&&Auth::user()->userType=='client' && $item->handoverStatus=='unconfirmed')
            <form action="/client/handover" method="POST">
              @csrf
                <input type="hidden" value="{{$item->delivery4_id}}" name="delivery_4_id">
