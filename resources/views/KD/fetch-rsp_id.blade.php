...
<body class="sb-nav-fixed">
    ...
    <div id="layoutSidenav">
                    ...
                    <div class="container px-3 my-5 clearfix">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h1>Delivery handing over to rom</h1>
                            </div>
                    <div class="card-body">
            ...
            <form action="{{ ('/key_distro/Handover_to_rom/post') }}" method="POST">
             ...
             <input type="hidden" value="{{ $hierarchy->id }}" name="hierarchy_id">
                @endforeach
            <input type="hidden" value="{{ $rsp[0]->user_id }}" name="rsp_user_id">


                    <label> You are handing over for : <strong> {{ $rom[0]->firstName }} {{ $rom[0]->middleName }} {{ $rom[0]->lastName }}</strong></label> <br>
                 @if(!isset($item))
                    <?php header("location:javascript://history.go(-1)"); ?>
                @endif
                 @if(isset($item))
                   <button <?php if ($item->quantity > $item->attributes->ordered_quantity){?> disabled <?php  }?> type="submit" class="btn btn-outline-success mt-2">Proceed</button>
                @endif
                 </form>
