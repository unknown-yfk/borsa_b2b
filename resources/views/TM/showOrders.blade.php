                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="table-responsive pt-3">
                            <form action="{{ route('tmShow') }}" method="GET" class="filter-form" id="filterForm">
                            ...
                            <form method="post" action="{{ route('tm_confirm') }}">
                                @csrf
                                                ...
                                                @for ($j = 0; $j < sizeof($product); $j++)
                                                    ...
                                                    @for ($r = 0; $r < sizeof($products_id); $r++)
                                                        @if ($product[$j]->id == $products_id[$r])
                                                            @php
                                                            $price_updates = explode(',', $client[$i]->price_updates);
                                                            $status_order = explode(',',$client[$i]->status);
                                                            if($price_updates[$r]==1)
                                                            {

                                                               if($status_order[$r]=="quantity_adjustment")
                                                                {

                                                                $subPrice = explode(',', $client[$i]->product_price);
                                                                $quantities = explode(
                                                                    ',',
                                                            ...
                                                            else
                                                            {
                                                                ...
                                                                $price = $amount*($subPrice[$r]/$quantities_orded[$r]);
                                                                ...
                                                                $quantities = explode(',',$client[$i]->ordered_quantities);
                                                                $amount = $quantities[$r];
                                                                $price = $amount*($subPrice[$r]/$amount);
                                                            ...
                                                            @endphp
                                                             ...
                                                             <input type="hidden" {{ $order_total_single = $order_total_single + $price }} />
                            ...
                            <button type="submit" class="savebutton btn-primary">Confirm</button>
