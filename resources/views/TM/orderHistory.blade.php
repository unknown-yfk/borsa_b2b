            ...
            <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <input type="hidden" {{ $order_total_single = 0 }} />
                        <input type="hidden" {{ $size = sizeof($product) }} />

                        <div class="table-responsive pt-3">
                            <form action="{{ route('tmorderhistory') }}" method="post" class="filter-form" id="filterForm">
                                @csrf
                                <div class="region-select">

                                    <button type="button" id="showRegionDropdownBtn" class="bg-white drop-button ">Select by
                                        Regions</button>
                                    ...
                                    <input type="hidden" value={{ $rom_id }} id="rom_id" name="rom_id" />
                                </div>
                            ...
                            <form method="post" action="{{ route('tm_handover') }}">
                                            ...
                                            @for ($i = 0; $i < sizeof($client); $i++)
                                                <tr>
                                                    <td style="width: 35px; white-space: normal; word-wrap: break-word;">
                                                        <input type="hidden" name="orders[{{ $i }}][orderId]"
                                                            value="{{ $client[$i]->id }}" />
                                                        {{ $client[$i]->id }}</td>
                                                    ...
                                                    @for ($j = 0; $j < sizeof($product); $j++)
                                                        ...
                                                        @for ($r = 0; $r < sizeof($products_id); $r++)
                                                            @if ($product[$j]->id == $products_id[$r])
                                                                @php
                                                                    $price_updates = explode(
                                                                        ',',
                                                                        $client[$i]->price_updates,
                                                                    );
                                                                    $status_order = explode(',', $client[$i]->status);
                                                                    if ($price_updates[$r] == 1) {
                                                                        if (
                                                                            $status_order[$r] == 'quantity_adjustment'
                                                                        ) {
                                                                            $subPrice = explode(
                                                                                ',',
                                                                                $client[$i]->product_price,
                                                                            );
                                                                            $quantities = explode(
                                                                                ',',
                                                                                $client[$i]->kd_adjusted_quantities,
                                                                            );

                                                                            $amount = $quantities[$r];
                                                                            $price = $amount * $subPrice[$r];
                                                                        } else {
                                                                            $subPrice = explode(
                                                                                ',',
                                                                                $client[$i]->product_price,
                                                                ...
                                                                @endphp
                                                                <input type="hidden"
                                                                    {{ $order_total_single = $order_total_single + $price }} />
                                                            @break
                                                ...
                                                @endfor
                                                <td style="width: 55px;">{{ $order_total_single }}</td>
                                            </tr>
                                            <input type="hidden" {{ $total = $total + $order_total_single }} />
                                            <input type="hidden" {{ $order_total_single = 0 }} />
                                        @endfor
                                    @endif
                            ...
                            <select id="hierarchy" name="hierarchy" class="form-control " required>
                                ...
                                @foreach ($hierarchy as $hierarchy)
                                    <option value="{{ $hierarchy->id }}"> <strong>{{ $hierarchy->name }} </strong>
                                    </option>
                                @endforeach


                            </select><br />
                            <input type="hidden" value="{{ $rom_id }}" name="rom_id">
...
<script>
    $(document).ready(function() {
        ...
        // Show/hide dropdown when the button is clicked
        $('#showRegionDropdownBtn').on('click', function() {
            isDropdownVisible = !isDropdownVisible;
